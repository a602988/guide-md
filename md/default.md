...
1. [預約項目設定](/guide/reservation#預約項目設定)：`預約設定->預約項目`：設定販售的商品對應預約預約項目，包含場次設定等等。
1. [設定場次群組](/guide/reservation#設定場次群組)：`預約設定->場次群組`：設定預約項目的場次，能將以群組化，當不同的預約項目相同場次時間時，不需重新建立，直接選用即可。
1. [建立場次日期](/guide/reservation#設定場次群組)：`預約設定->場次日期`：設定場次群組適用的預約日期區間。
1. [設定預約位置](/guide/reservation#設定場次群組)：`控制台->參數項目`：此依據各別專案建置的項目，用於課程、活動位置或教室等建立。
1. [建立場次人數](/guide/reservation#設定場次群組)：`預約設定->場次人數`：設定場次群組的各別時段與時段的人數，可用[日曆檢視場次人數](/guide/reservation-number#建立場次人數)。

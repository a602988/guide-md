...
1. 於 `訂單管理->金流資料` 列表中篩選，或依據建立的報表 [已付款-信用卡](/guide/order-payment#已付款-信用卡) ，撈取後，點選需要異動金流資料 `編輯`
1. 可於表上核對，或可匯出核對：請參見 [匯出金流資料](/guide/order-payment#匯出金流資料)
2. 確認無誤後，於 選右上角按鈕點選 `異動多重狀態` 功能
3. 於彈跳視窗中，設定以下異動狀態
...
1. 於 `訂單管理->金流資料` 列表中篩選，或依據建立的報表 [已付款-虛擬帳號ATM](/guide/order-payment#已付款-信用卡) ，撈取後，點選需要異動金流資料 `編輯`
1. 可於表上核對，或可匯出核對：請參見 [匯出金流資料](/guide/order-payment#匯出金流資料)
2. 確認無誤後，於 選右上角按鈕點選 `異動多重狀態` 功能
3. 於彈跳視窗中，設定以下異動狀態
